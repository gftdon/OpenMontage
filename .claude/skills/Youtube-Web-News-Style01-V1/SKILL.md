...
description: >-
  ...
  "ทำเหมือนคลิป Fable 5.1 / Claude news", or any avatar mp4 + SRT + article URL
...
- Overlays: AI NEWS top bar + amber kicker per beat, number/info chips that pop
  on word times, BREAKING lower-third on the cold open, `claude.ai`-style CTA
...
from `~/node_modules` or global (`cd ~ && npm i playwright@latest && npx
...
```bash
python3 .claude/skills/Youtube-Web-News-Style01-V1/scripts/srt_to_script.py --srt SOURCE.srt --out projects/<slug>/artifacts/script.txt
...
```bash
python3 .claude/skills/Youtube-Web-News-Style01-V1/scripts/line_starts.py \
...
python3 .claude/skills/Youtube-Web-News-Style01-V1/scripts/words_by_line.py \
...
- Stat chips, highlight reveals, chip lists, CTA = the `s` of the exact word
  (`25%`, `Millennium`, `สรุป`, `claude.ai`…) from the word dump.
...
```bash
node .claude/skills/Youtube-Web-News-Style01-V1/scripts/explore_page.mjs "<URL>"   # h1/h2 positions, tabs, buttons, <video> sources
...
```bash
node .claude/skills/Youtube-Web-News-Style01-V1/scripts/capture_page.mjs projects/<slug>/artifacts/capture-config.json
...
```bash
.venv/bin/python .claude/skills/Youtube-Web-News-Style01-V1/scripts/fetch_broll.py projects/<slug>/artifacts/broll-queries.json projects/<slug>/assets/broll --music "technology corporate inspiring background" projects/<slug>/assets/audio/bed.mp3
bash .claude/skills/Youtube-Web-News-Style01-V1/scripts/transcode_broll.sh projects/<slug>/assets/broll remotion-composer/public/<mediaDir>/broll
```
...
`anthropic`, `claude.ai`) before rendering — anything left outside `EPISODE`
...
```bash
node .claude/skills/Youtube-Web-News-Style01-V1/scripts/stills.mjs <CompId> 75,388,1320,... remotion-composer/out/qa remotion-composer
```
...
bash .claude/skills/Youtube-Web-News-Style01-V1/scripts/mux_av.sh remotion-composer/out/<name>_visual.mp4 AVATAR.mp4 projects/<slug>/assets/audio/bed.mp3 projects/<slug>/renders/<name>.mp4 <TOTAL> <VO_END>
```
